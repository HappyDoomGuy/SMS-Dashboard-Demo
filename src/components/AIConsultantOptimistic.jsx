...
const AIConsultantOptimistic = ({ data, contentType, campaignsData, clientsData }) => {
  ...
  const prepareDataForAnalysis = () => {
    ...
    // A/B тест анализ: группируем по тексту SMS и A/B группе
    const abTestResults = {};
    data.forEach(item => {
      const smsText = item.smsText || 'Текст не найден';
      const abGroup = item.abGroup || 'Без группы';
      const campaignName = item.campaignName || 'Без кампании';
    ...
    // Преобразуем в массив с вычисленными средними
    ...
    return {
      ...
      dayOfWeekStats,
      hourlyStats,
      abTestAnalysis, // Добавляем A/B анализ
      dateRange: {
  ...
  const analyzeData = async () => {
    ...
    try {
      ...
      const prompt = `Ты — маркетинговый консультант-стратег фармацевтической индустрии, специализирующийся на оптимизации SMS-рассылок. Твоя задача — проанализировать эффективность текстов SMS из A/B тестов и дать ПРАКТИЧЕСКИЕ РЕКОМЕНДАЦИИ для препарата "${dataForAnalysis.contentType}".

**КОНТЕКСТ A/B ТЕСТИРОВАНИЯ:**
- Каждая кампания начинается с A/B тестирования (группы: A, B, C и т.д.)
- У каждой группы свой вариант текста SMS
- После A/B теста проводится общая рассылка (группа "Общее") с текстом, который показал лучшие результаты
- В данных есть информация о группах A/B и текстах SMS для каждой группы
...
- ✅ АНАЛИЗИРУЙ тексты SMS из разных A/B групп
- ✅ СРАВНИВАЙ какие элементы текста работают лучше
- ✅ ВЫЯВЛЯЙ паттерны в успешных SMS
- ✅ ДАВАЙ рекомендации по написанию текстов SMS
- ✅ Предлагай конкретные формулировки и структуры
- ✅ Используй формат: "Для повышения X рекомендую использовать Y в тексте"
...
**🧪 РЕЗУЛЬТАТЫ A/B ТЕСТИРОВАНИЯ SMS (КЛЮЧЕВЫЕ ДАННЫЕ):**
${dataForAnalysis.abTestAnalysis.map((test, i) => 
  `${i + 1}. Кампания: "${test.campaignName}"
   Группа A/B: ${test.abGroup}
   Текст SMS: "${test.smsText}"
   Просмотров: ${test.views}
   Среднее время: ${Math.floor(test.avgTime / 60)}м ${test.avgTime % 60}с
   Средний % просмотра: ${test.avgViewPercent}%`
).join('\n\n')}

**ЗАДАНИЕ:**
На основе РЕЗУЛЬТАТОВ A/B ТЕСТИРОВАНИЯ дай рекомендации в СТРОГОМ ПОРЯДКЕ:

**РАЗДЕЛ 1: НОВЫЕ ВАРИАНТЫ ТЕКСТОВ SMS** 📝

**A) 5 вариантов с высокой вероятностью повышения конверсии:**
- Проанализируй SMS с ЛУЧШИМИ показателями (больше просмотров, выше % просмотра)
- Выяви ЗАКОНОМЕРНОСТИ: что делает их успешными (структура, слова, длина, призыв, тон)
- Создай 5 НОВЫХ текстов, применяя эти ЗАКОНОМЕРНОСТИ
- НЕ копируй старые тексты, создавай ОРИГИНАЛЬНЫЕ
- Каждый вариант имеет ВЫСОКУЮ ВЕРОЯТНОСТЬ улучшить конверсию
- Формат для каждого:
  **Вариант 1:**
  [полный новый текст SMS]
  *Закономерность:* [какая выявленная закономерность использована]
  *Вероятность:* [почему этот вариант скорее всего улучшит конверсию]

**B) 5 экспериментальных вариантов (новизна):**
- Предложи ПРИНЦИПИАЛЬНО НОВЫЕ подходы, которых НЕТ в существующих SMS
- Анализируй использованные SMS и создавай ОТЛИЧАЮЩИЕСЯ варианты
- Каждый вариант должен тестировать ЧТО-ТО НОВОЕ:
  * Новая структура сообщения
  * Новый тон коммуникации
  * Новый тип призыва к действию
  * Новый формат подачи информации
  * Новые эмоциональные триггеры
- НЕ повторяй подходы из уже использованных SMS
- Смелые, инновационные идеи для поиска новых решений
- Формат для каждого:
  **Вариант 6:**
  [полный новый текст SMS - НЕПОХОЖИЙ на существующие]
  *Эксперимент:* [что НОВОГО тестируем, чего не было]
  *Гипотеза:* [почему это может сработать лучше]

**РАЗДЕЛ 2: АНАЛИЗ ЛИДИРУЮЩИХ SMS ТЕКСТОВ** 📊

Проанализируй самые успешные SMS из A/B тестов:
- Какие SMS показали лучшие результаты
- ЗАКОНОМЕРНОСТИ в их структуре и содержании
- Что их объединяет (общие элементы)
- Какие принципы делают их эффективными
- Конкретные примеры успешных элементов

**РАЗДЕЛ 3: ОБЩИЕ РЕКОМЕНДАЦИИ ПО КОНВЕРСИИ** 💡

На основе анализа ВСЕЙ конверсии:
- Ключевые факторы, влияющие на конверсию
- Паттерны поведения аудитории
- Рекомендации по времени отправки
- Персонализация для разных специальностей
- Стратегические принципы для роста конверсии

КРИТИЧЕСКИ ВАЖНО: 
- ПОРЯДОК: Сначала 10 вариантов → Затем анализ лидеров → Затем общие рекомендации
- ВЫЯВЛЯЙ закономерности в успешных SMS
- Первые 5 вариантов (1-5): применяют ПРОВЕРЕННЫЕ закономерности → высокая вероятность успеха
- Следующие 5 вариантов (6-10): ЭКСПЕРИМЕНТАЛЬНЫЕ, НЕ похожие на существующие SMS
- Экспериментальные варианты должны ОТЛИЧАТЬСЯ по подходу от всех использованных ранее
- Все тексты ОРИГИНАЛЬНЫЕ, НЕ копии
- Используй ДАННЫЕ для выявления закономерностей И для понимания что НЕ было опробовано`;
          ...
          {!analysis && !loading && !error && (
            ...
            >
              ...
              <Typography variant="h6" sx={{ mb: 1, fontWeight: 800, color: '#1a2332' }}>
                Готов проанализировать тексты SMS "{contentType}"
              </Typography>
              <Typography variant="body2" sx={{ mb: 4, color: '#6b7280', maxWidth: '400px', lineHeight: 1.6 }}>
                Получите рекомендации по текстам SMS на основе A/B тестирования и практические советы по улучшению
              </Typography>
              <Button
                variant="contained"
                size="large"
          ...
          {loading && (
            ...
            >
              ...
              <Typography variant="h6" sx={{ mb: 1, mt: 4, color: '#1a2332', fontWeight: 700 }}>
                Анализирую тексты SMS...
              </Typography>
              <Typography variant="body2" sx={{ color: '#6b7280' }}>
                ИИ сравнивает результаты A/B тестов и готовит рекомендации
              </Typography>
            </Box>
          )}
